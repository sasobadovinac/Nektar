    ...
    try
    {
        ...
        for(int v = 0; v < nvars; ++v)
        {
            ...
            if (ex_sol)
            {
                ...
                int coordim = Exp[0]->GetCoordim(0);
                vector<NekDouble> vQError(coordim);

                for (int i = 0; i < coordim; ++i)
                {
                    Exp[v]->PhysDeriv(i, Fce.GetArray1D(), tmp);
                    vQError[i] = std::dynamic_pointer_cast
                        <MultiRegions::DisContField>(Exp[v])->L2_DGDeriv
                        (i, Coeffs.GetArray1D(), tmp);
                }
                ...
                if (vSession->GetComm()->GetRank() == 0)
                {
                    ...
                    for (int i = 0; i < coordim; ++i)
                    {
                        cout << "Q" << i << " L2 error  (variable "<< var << ") : "
                             << vQError[i] << endl;
                    }
                }
            }
