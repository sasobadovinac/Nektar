...
struct IProductTemplate : public IProduct,
                          public Helper<LibUtilities::ShapeTypeDimMap[SHAPE_TYPE], DEFORMED>
{
    IProductTemplate(std::vector<LibUtilities::BasisSharedPtr> basis,
                  int nElmt)
    ...
    static std::shared_ptr<Operator> Create(
        ...
        int nElmt)
    {
      return std::make_shared<IProductTemplate<SHAPE_TYPE, DEFORMED>>(basis, nElmt);
    }
...
#include "SwitchNodesPoints.h"
                          ...
                          Array<OneD,       NekDouble>& output)
    {
        ...
        std::vector<vec_t, allocator<vec_t>> wsp0(wsp0Size), tmpIn(nqTot), tmpOut(m_nmTot);
