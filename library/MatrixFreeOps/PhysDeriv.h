...
struct PhysDerivTemplate : public PhysDeriv,
                           public Helper<LibUtilities::ShapeTypeDimMap[SHAPE_TYPE], DEFORMED>
{
    ...
    static std::shared_ptr<Operator> Create(
        ...
        int nElmt)
    {
      return std::make_shared<PhysDerivTemplate<SHAPE_TYPE, DEFORMED>>(basis, nElmt);
    }
...
#include "SwitchPoints.h"
    ...
    {
        ASSERTL1(output.size() >  0, "PhysDerivTemplate::Operator1D: Cannot call 1D routine with no output.");
        ASSERTL1(output.size() <= 3, "PhysDerivTemplate::Operator1D: Operator not set up for other dimensions.")
        ...
        for (int e = 0; e < this->m_nBlocks; ++e)
        {
            ...
            PhysDeriv1DKernel(nq0, ndf, tmpIn, this->m_D[0], df_ptr, df_tmp, tmpOut);
    ...
    void operator2D(const Array<OneD,       const NekDouble> & input,
                          Array<OneD, Array<OneD, NekDouble>>& output)
    {
        ASSERTL1(output.size() > 1,  "PhysDerivTemplate::Operator2D: Cannot call 2D routine with one output.");
        ASSERTL1(output.size() <= 3, "PhysDerivTemplate::Operator2D: Operator not set up for 3D coordinates.");
        ...
        for (int e = 0; e < this->m_nBlocks; ++e)
        {
            ...
            PhysDeriv2DKernel(nq0, nq1,
                              outdim, tmpIn,
                              this->m_Z[0], this->m_Z[1],
                              this->m_D[0], this->m_D[1],
                              df_ptr, df_tmp, tmpOut);
    ...
    void operator3D(const Array<OneD,       const NekDouble> & input,
                          Array<OneD, Array<OneD, NekDouble>>& output)
    {
        ASSERTL1(output.size() == 3,"PhysDerivTemplate::Operator3D: Cannot call 3D routine with 1 or 2 outputs.");
        ...
        PhysDeriv3DWorkspace<SHAPE_TYPE>(nq0, nq1, nq2,
                             wsp0Size, wsp1Size);
            ...
            for (int k = 0, eta0 = 0; k < nq2; ++k)
            {
                ...
                for (int j = 0; j < nq1; ++j)
                {
                    for (int i = 0; i < nq0; ++i, ++eta0)
                    {
                        ...
                        wsp0[eta0] = out0; // 2 * (1 + eta_0) / (1 - eta_1)(1-eta2) | store 1x
