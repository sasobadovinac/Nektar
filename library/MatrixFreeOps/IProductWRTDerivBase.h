...
{
    IProductWRTDerivBaseTemplate(std::vector<LibUtilities::BasisSharedPtr> basis,
                  int nElmt)
    : IProductWRTDerivBase(basis, nElmt),
      Helper<LibUtilities::ShapeTypeDimMap[SHAPE_TYPE], DEFORMED>(basis, nElmt)
    {
    ...
    static std::shared_ptr<Operator> Create(
        ...
        int nElmt)
    {
      return std::make_shared<IProductWRTDerivBaseTemplate<SHAPE_TYPE, DEFORMED>>(basis, nElmt);
    }
    ...
    {
        ASSERTL1(input.size() >  0, "IProductWRTDerivBaseTemplate::Operator1D: Cannot call 1D routine with no input.");
        ASSERTL1(input.size() <= 3, "IProductWRTDerivBaseTemplate::Operator1D: Operator not set up for other dimensions.")
        ...
        std::vector<vec_t, allocator<vec_t>> tmpIn[max_ndf]; // max_ndf is a constexpr
        ...
        for (int e = 0; e < this->m_nBlocks; ++e)
        {
            ...
            IProductWRTDerivBase1DKernel<SHAPE_TYPE, DEFORMED>(
                                         nqTot, ndf,
                                         df_ptr, df_tmp, tmpIn, tmp0);

            // IP DB0
            IProduct1DKernel<SHAPE_TYPE, false, false, DEFORMED>(nm0, nq0,
                             tmp0, this->m_dbdata[0], this->m_w[0],
                             jac_ptr, tmpOut);
    ...
    void operator2D( const Array<OneD, Array<OneD, NekDouble>> &input,
                           Array<OneD,             NekDouble> &output)
    {
        ASSERTL1(input.size() > 1,  "IProductWRTDerivBaseTemplate::Operator2D: Cannot call 2D routine with one output.");
        ASSERTL1(input.size() <= 3, "IProductWRTDerivBaseTemplate::Operator2D: Operator not set up for 3D coordinates.");
        ...
        std::vector<vec_t, allocator<vec_t>> tmpIn[max_indim],  // max_indim is a constexpr
    ...
    void operator3D( const Array<OneD, Array<OneD, NekDouble>> &input,
                           Array<OneD,             NekDouble> &output)
    {
        ASSERTL1(input.size() == 3,"IProductWRTDerivBaseTemplate::Operator3D: Cannot call 3D routine with 1 or 2 outputs.");
        ...
        IProduct3DWorkspace<SHAPE_TYPE>(nm0, nm1, nm2, nq0, nq1, nq2,
                            wsp0Size, wsp1Size, wsp2Size);

        std::vector<vec_t, allocator<vec_t>> wsp0(wsp0Size), wsp1(wsp1Size), wsp2(wsp2Size),
        ...
        for (int e = 0; e < this->m_nBlocks; ++e)
        {
            ...
            IProduct3DKernel<SHAPE_TYPE, false, false, DEFORMED>(
                             nm0, nm1, nm2, nq0, nq1, nq2, correct,
