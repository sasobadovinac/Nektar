...
#define jacobz(n,z,alpha,beta) JacZeros(n,z,alpha,beta)
  ...
  */
   ...
   *
   */
  // Nektar::NekMatrix<Nektar::NekDouble> JacobiPoly(
  // 						  int degree,  
  // 						  double* x,  
  // 						  int P,
  // 						  double alph , 
  ...
  //   Nektar::Array<Nektar::OneD,Nektar::NekDouble >Vr((degree+1)*v2);
  //   Nektar::Array<Nektar::OneD,Nektar::NekDouble >Vrd((degree+1)*v2);
  //   Nektar::Array<Nektar::OneD,Nektar::NekDouble >Vrd2((degree+1)*v2);

  //   std::vector<Nektar::NekDouble> jacobi_i1(v2*(degree+1));
  //   std::vector<Nektar::NekDouble> jacobi_i3(v2*(degree+1));
  //   std::vector<Nektar::NekDouble> jacobi_i2(v2*(degree+1));
  //   Polylib::jacobfdd(v2, &x[0], &jacobi_i1[0],  &jacobi_i2[0],  &jacobi_i3[0], degree, alph, bet);
  //   for(int m = 0; m < degree+1; m++)
  //     {

  // 	Vmath::Vcopy(v2,&jacobi_i1[0]+m,(degree+1),&Vr[0]+m*v2,1); 
	    
  //     }
  //   Vrd = Nektar::Array<Nektar::OneD,Nektar::NekDouble >((degree+1)*v2,&jacobi_i2[0]);
  //   Vrd2 = Nektar::Array<Nektar::OneD,Nektar::NekDouble >((degree+1)*v2,&jacobi_i3[0]);
  //   int v1 = degree+1;
  //   if(d == 0)
  //     {
  // 	Nektar::NekDouble *t = Vr.data();
  // 	Nektar::NekMatrix<Nektar::NekDouble> Vr2(v2,v1,t);
  // 	return Vr2;
  //     }
  //   else if( d == 1)
  //     {
 
  // 	Nektar::NekDouble *td = Vrd.data();
  // 	Nektar::NekMatrix<Nektar::NekDouble> Vr2(v2,v1,td);
	    
  // 	return Vr2;
  //     }
  //   else
  //     {
  // 	std::cout<<"\n invalid derivative d="<<d<<" !\n\n";
  // 	exit(0);
  //     }
  ...
  // Nektar::NekMatrix<Nektar::NekDouble> PolyEval2(
  // 						 int Ndeg,	
  // 						 double* xa,  
  // 						 double* ya, 
  // 						 int P1,
  ...
  // 						 int d1, int d2)
  // {	
  //   const int s1 = P1;//xa.num_elements();
  //   const int s2 = P2;//ya.num_elements();
	
  //   ASSERTL0(s1 == s2 || ((s1 == 1 && s2!= 1) || ((s1 != 1 ) && s2 == 1)) ,"failed in poly_eval2" );
  ...
  //   int qq,rr, sz = pow(Ndeg+1,2);
  //   Nektar::NekMatrix<Nektar::NekDouble > LL (evalPtsin,sz);

  //   Nektar::Array<Nektar::OneD,Nektar::NekDouble> t2(sz);
  //   Nektar::NekMatrix<Nektar::NekDouble> R1, R2, 
  //     RetR1(evalPtsin, Ndeg+1), RetR2(evalPtsin,Ndeg+1);
  //   R1= JacobiPoly(Ndeg, &xa[0], s1, alph, bet, d1); 
  //   if( s1 != s2 && s1 == 1)
  //     {
  // 	for(i = 0; i < Ndeg+1; i++)
  // 	  {
  // 	    Nektar::Array<Nektar::OneD,Nektar::NekDouble> temp(evalPtsin, R1(0,i));
  // 	    Vmath::Vcopy(evalPtsin, &temp[0], 1, &RetR1.GetPtr()[0]+(i*evalPtsin),1);
  // 	  }
  //     }
  //   else
  //     {
  // 	RetR1 = R1;
  //     }
  //   R2 = JacobiPoly(Ndeg, &ya[0], s2, alph, bet, d2);	
  //   if( s1 != s2 && s2 == 1)
  //     {
  // 	for(i = 0; i < Ndeg+1; i++)
  // 	  {
  // 	    Nektar::Array<Nektar::OneD,Nektar::NekDouble> temp(evalPtsin, R2(0,i));
  // 	    Vmath::Vcopy(evalPtsin, &temp[0], 1, &RetR2.GetPtr()[0]+(i*evalPtsin),1);
  // 	  }
  //     }
  //   else
  //     {
  // 	RetR2 = R2;
  //     }
  //   for(i = 0; i<evalPtsin; i++)
  //     {       
  // 	int ctr = 0;
  // 	for(rr = 0; rr<Ndeg+1; rr++)
  // 	  {
  // 	    for(qq = 0; qq<Ndeg+1; qq++)
  // 	      { 
  // 		Nektar::NekDouble v1 = 1.0*RetR1(i,qq);
  // 		Nektar::NekDouble v2 = 1.0*RetR2(i,rr);
  // 		t2[ctr++] = (v1*v2);
  // 	      }
		
  // 	  }
  // 	Vmath::Vcopy(sz,&t2[0],1,&LL.GetPtr()[0]+(i),evalPtsin);
  //     }
  //   // 	M = &LL.GetPtr()[0];
  //   return LL;
  // }
  ...
  */
  ...
  void Dgj2(double *D,  const double *z, const int np, const int n,const double alpha,
	    const double beta)
  {
    double two = 2.0;
    ...
    if (np <= 0){
      ...
      pd = (double *)malloc(np*sizeof(double));
      jacobd(np,z,pd,n,alpha,beta);

      for (i = 0; i < n; i++){
	for (j = 0; j < np; j++){

	  //if (i != j) 
	  //     D[i*n+j] = pd[j]/(pd[i]*(z[j]-z[i]));
	  // else    
	  D[i*n+j] = (alpha - beta + (alpha + beta + two)*z[j]);
	  //                       (two*(one - z[j]*z[j]));
	}
      }
      free(pd);
  ...
  */
  void jacobfd(const int np, const double *z, double *poly_in, double *polyd, 
	...
	std::vector<double> Datm1d {0,1.22474487139159,-4.74341649025257,11.2249721603218,-21.2132034355964,35.1781181986757,-53.5397048927242,76.6811580507232,-104.957134107215,138.699315066802,-178.220369206216,223.816889443134,-275.771644662754,334.355349889904,-399.828088057856,472.440472440709};
	a1 = n*(alpha - beta);
  ...
  void jacobfdd(const int np, const double *z, double *poly_in, double *polyd, 
		double *polydd,const int n, const double alpha, const double beta){
    register int i;
    double  zero = 0.0, one = 1.0, two = 2.0;
    double   *poly, *polyn1,*polyn2 ;
    std::vector<double> temp(np*(n+1));
    double   a1,a2,a3,a4, scal;
	    
    if(alpha == 0.0 && beta == 0.0)
      scal = 1.0;
    else //only handles chebyshev
      scal = (1/sqrt(M_PI));
	...
	if(ctr == 0){
	  ...
	  if(poly_in)
	    for(i = 0; i < np; ++i) 
	      poly_in[i*(n+1)] = scal*one;
	  if(polyd)
	    for(i = 0; i < np; ++i) 
	      polyd[i*(n+1)] = zero; 
	  if(alpha == -0.5 && beta == -0.5)
	    {   
	      scal *= (1/sqrt(0.5))*(4.0*(ctr+1.0)*(ctr+1.0)/(2.0*ctr+2.0)/(2.0*ctr+1.0));
	    }
	}
	else if (ctr == 1){
	  if(alpha == 0.0 && beta == 0.0)
	    scal = sqrt(0.5*(2.0*ctr+1.0));	    
  

	  if(poly_in)
	    for(i = 0; i < np; ++i) 
	      poly_in[i*(n+1)+ctr] = scal*0.5*(alpha - beta + (alpha + beta + two)*z[i]);
	  if(polyd)
	    for(i = 0; i < np; ++i) 
	      polyd[i*(n+1)+ctr] = scal*0.5*(alpha + beta + two);
	  if(alpha == -0.5 && beta == -0.5)
	    {
	      scal *= (4.0*(ctr+1.0)*(ctr+1.0)/(2.0*ctr+2.0)/(2.0*ctr+1.0));
	    }
	}
	else{
	  register int k;
	  ...
	  if(poly_in){ // switch for case of no poynomial function return
	    polyn1 = (double *)malloc(2*np*(n+1)*sizeof(double));
	    polyn2 = polyn1+np*(n+1); 
	    poly   = poly_in;
	  }
	  else{
	    polyn1 = (double *)malloc(3*np*(n+1)*sizeof(double));
	    polyn2 = polyn1+np*(n+1); 
	    poly   = polyn2+np*(n+1);      
	  }
		  
	  for(i = 0; i < np; ++i){
	    polyn2[i*(n+1)+ctr] = one;
	    polyn1[i*(n+1)+ctr] = 0.5*(alpha - beta + (alpha + beta + two)*z[i]);
	  }
	  for(k = 2; k <= ctr; ++k){
	    a1 =  two*k*(k + apb)*(two*k + apb - two);
	    a2 = (two*k + apb - one)*(alpha*alpha - beta*beta);
		...
		double val = ((a2 + a3*z[i])*polyn1[i*(n+1)+ctr] - a4*polyn2[i*(n+1)+ctr]);
		if(k == ctr)
		  {
		...
		poly  [i*(n+1)+ctr] = val;
		polyn2[i*(n+1)+ctr] = polyn1[i*(n+1)+ctr];
		polyn1[i*(n+1)+ctr] = poly  [i*(n+1)+ctr];
		temp[i*(n+1)+ctr] = val;
	  ...
	  if(alpha == -0.5 && beta == -0.5) //only handles chebyshev
	    { 
	      scal *= (4.0*(ctr+1.0)*(ctr+1.0)/(2.0*ctr+2.0)/(2.0*ctr+1.0));
	    }
	  free(polyn1);
    ...
    if(polyd){
      ...
      std::vector<double> tp(np);
      std::vector<double> D(np*(n+1));
	...
	{	
	  ...
	  double scal = sqrt(0.5*(2.0*ctr+1.0));	    
	  jacobd(np, &z[0], &D[0]+ctr, ctr, alpha, beta);
	  Vmath::Smul(np, scal, &D[0]+ctr, 1, &polyd[0]+(np*ctr),1);
	      
	  if(alpha == -0.5 && beta == -0.5) //only handles chebyshev for now
	    scal = (4*(ctr+1)*(ctr+1)/(2*ctr+2)/(2*ctr+1));
    ...
    if(polydd){
      std::vector<double> D(np*(n+1));
      std::vector<double> D2(np*(n+1));
	      
      for(int uu = 0; uu<n-1; uu++)
	{
	  std::vector<double> t(np);
	  jacobfd(np, z, &t[0], NULL, uu, alpha+2, beta+2);
	  Vmath::Vcopy(np, &t[0], 1, &D[0]+uu+2, n+1);
	}
	...
	{	
	  ...
	  double num = gammaF(alpha+beta+ctr+1+2);
	  double den = gammaF(alpha+beta+ctr+1);
	  double den2 = pow(2.0,2)*den;
	  Vmath::Smul(np, scal*(num/den2), &D[0]+ctr, (n+1), &D2[0]+ctr, (n+1));
	  Vmath::Vcopy(np, &D2[0]+ctr, n+1, &polydd[0]+(np*ctr), 1);
