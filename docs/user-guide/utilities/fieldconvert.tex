...
\item \inltt{addConstToFld}: Add a constant value to a .fld file;
...
\subsection{Add a constant to a .fld file: \textit{addConstToFld} module}
To add a constant to a .fld file one can use the \inltt{addconsttofld}
module of FieldConvert
%
\begin{lstlisting}[style=BashInputStyle]
  FieldConvert -m addconsttofld:value=const:fieldid=2 file1.xml file1.fld \
  file2.fld
\end{lstlisting}
%
In this case we use it in conjunction with the command \inltt{fieldid}
which specifieds the id of the field to which we can add the constant. 
\inltt{file1.fld} is the file on which we want to add the constant of value
\inltt{const}, \inltt{file1.xml} is the associated session file, \inltt{file2.fld} is the output which contain the modified data. 
\inltt{file2.fld} can be processed in a similar way as described
