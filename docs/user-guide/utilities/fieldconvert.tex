...
\item \inltt{divergence}: Computes the divergence of the velocity field.
...
\item inltt{bodyFittedVelocity}: Generate a body-fitted coordinate system and then 
...
\end{lstlisting}
where the file \inltt{test-div.fld} can be processed in a similar
way as described in section \ref{s:utilities:fieldconvert:sub:convert}.
%
%
%
...
\end{lstlisting}
%
where \inltt{vars} specifies the field numbers (starting from 0) or variable names to process and
\inltt{vars} specifies the spatial directions to compute derivatives. All fields will be
processed if \inltt{vars} is not set. All spatial directions will be used if \inltt{vars} is not set.
The output file \inltt{file-grad.fld} can be processed in a similar
way as described in section \ref{s:utilities:fieldconvert:sub:convert}
to visualise the result either in Tecplot, Paraview or VisIt.
%
%
...
(\inltt{Shear\_x}), the vertical direction (\inltt{Shear\_y}) and its magnitude
...
\end{lstlisting}
The option \inltt{bnd} specifies the reference boundary composite to set up
...
In general, when the surface is smooth, \inltt{checkAngle} and \inltt{distTol} is
...
Finally, \inltt{bfcOut} is a bool parameter controls whether the fields for body-fitted 
coordinate system will be output. This file can be read by the \inltt{BodyFittedVelocity}
...
\inltt{.dat} and \inltt{.plt} outputs the current version will produce
...
%
%
\section{FieldConvert for parallel-in-time}
To run FieldConvert in parallel the user needs to compile
\nekpp with MPI support and can employ the following
command
\begin{lstlisting}[style=BashInputStyle]
  mpirun -np <nprocs> FieldConvert --npt <nprocs> test.xml \
                 in/test\_<n>.fld out/test\_<n>.vtu
\end{lstlisting}
\begin{lstlisting}[style=BashInputStyle]
  mpirun -np <nprocs> FieldConvert --npt <nprocs> test.xml \
                 in/test\_<n>.fld out/test\_<n>.dat
\end{lstlisting}
replacing \inltt{<nprocs>} with the number of processors and \inltt{<n>} 
by the starting time index. FieldConvert will then process in parallel 
time solutions \inltt{test\_<n>}.dat to \inltt{test\_<n>+<nprocs>}.dat.
%
%
%
