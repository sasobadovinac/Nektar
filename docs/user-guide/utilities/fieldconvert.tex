...
If you want to build a version of \nekpp with integrated VTK library support, you
need  to turn on the \inltt{NEKTAR\_USE\_VTK} switch in cmake. This will then
...
command line option \inltt{uncompress} when using \inltt{FieldConvert}.

\begin{lstlisting}[style=BashInputStyle]
FieldConvert session.xml field.fld output.vtu:vtu:uncompress
\end{lstlisting}

If \nekpp has been compiled with \inltt{NEKTAR\_USE\_VTK} enabled then there are certain
...
such as 2DH2D will need to be converted using the \inltt{legacy} option to invoke the
manual \nekpp VTK output.
...
High-order output can be enabled using the option \inltt{highorder} which first
runs the mesh through the \inltt{equispacedoutput} module to get a truly
...
Multi-block output can be enabled using the option \inltt{multiblock} which splits
...
specified. It is also necessary to include the \inltt{CONDITIONS} section in the
session file supplied to \inltt{FieldConvert} because this is where the boundary
...
\dirtree{%
    ...
    .2 Domains.
    .3 Domain ID 0.
    .4 Left section unstructured.
    .4 Right section unstructured.
    .3 Domain ID 1.
    .4 Cylinder structured.
    .2 Boundaries.
    .3 Walls.
    .4 Top wall.
    .4 Bottom wall.
    ...
    .4 Right wall.
    .2 Other composites.
    .3 Cylinder inner interface.
    .3 Cylinder outer interface.
}
\end{minipage}
\end{multicols}
\end{center}
...
with \inltt{FieldConvert} to output a .vtu file, the mesh is now cached inbetween
...
\item \inltt{CFL}: Computes the CFL number over the solution domain for Incompressible flow
...
the outputs of \inltt{IncNavierStokesSolver}. To Estimate the \textit{CFL} 
...
where the file \inltt{test-cfl.fld} can be processed in a similar
way as described in section \ref{s:utilities:fieldconvert:sub:convert}
to visualise the result either in Tecplot, Paraview or VisIt.
%
%
...
In linear stability analysis with halfmode expansion, the variable w, whose variable number is 2, is a imaginary mode but it is output as a real mode in the eigenvector field file. By setting option \inltt{realmodetoimag=2}, w can be transformed to a imaginary mode correctly.
...
If the fromfld is a 3DH1D field and uses HalfMode expansion, you can use \inltt{realmodetoimag=$n_1,n_2,...,n_m$} to transform the $n_1,n_2,...,n_m$th variables from real modes to imaginary modes. Variable index starts from 0.
%
\begin{tipbox}
...
The output fields are wall shear stress in streamwise direction (\inltt{Shear\_s}),
normal direction (\inltt{Shear\_n}) and magnitude of the wall shear stress vector 
(\inltt{Shear\_mag}) for 2D simulations. For 3D simulations, wall shear stress in
the spanwise direction (\inltt{Shear\_z}) is also included in the output fileds.
Note that the streamwise and normal directions are local at each point on the 
...
\subsection{Postprocessing the data using the \textit{L-SIAC} filter}
The module implemented in this work, the Line-Smoothness Increasing Accuracy 
Conserving filter or L-SIAC filter, is derived from Jallepalli et al.
\cite{jallepalli2019adaptive,jallepalli2019efficient,jallepalli2017treatment}
. The L-SIAC filter can be applied using the following command:

\begin{lstlisting}[style=BashInputStyle]
FieldConvert -m LSIAC:configxml=config.xml input.xml input.fld output.fld
\end{lstlisting}

This command constructs the L-SIAC filter using the parameters defined in 
\inltt{config.xml}, postprocesses the fields defined by \inltt{input.xml} and 
\inltt{input.fld}, and writes them to \inltt{output.fld}.

An example of a \inltt{config.xml} file is shown here:
\begin{lstlisting}[style=XMLStyle]
<?xml version="1.0" encoding="utf-8" ?>
<LSIAC>
  <SET>
    <INVARIABLES> u,v,w </INVARIABLES>
    <OUTVARIABLES> Lu,Lv,Lw </OUTVARIABLES>
...
\end{lstlisting}
Using this config.xml file, the user can apply the L-SIAC filter on the fields 
u, v, w, and write them to Lu, Lv, and Lw, respectively. The rest of the values 
specify the parameters for the L-SIAC filter.

\begin{enumerate}
  \item \inltt{ORDER}: Specifies the polynomial order of the L-SIAC filter.
  \item \inltt{DIRECTION}: The default value is [1.0,0.0,0.0], which specifies 
the direction of the L-SIAC filter.
  \item \inltt{DERIVATIVE}: The default value is 0 (no derivative). It specifies
the order of the derivative to apply to the L-SIAC filter. The L-SIAC filter is 
implemented using the convolution operator; hence, the derivative is 
automatically applied to the fields.
\end{enumerate}
