...
\section{Converting a mesh to HDF5 format}
HDF5 meshes turn out to be more efficient than XML meshes when it comes to large cases since the time to generate the mesh graph at the beginning of the simulation is tremendously reduced. XML meshes can be converted to HDF5 meshes format using the following command:

\begin{lstlisting}[style=BashInputStyle]
    NekMesh meshXML.xml meshHDF5.nekg
\end{lstlisting}

Two files are created: \inltt{meshHDF5.nekg} containing the HDF5 mesh and \inltt{meshHDF5.xml} which is a short file indicating that the mesh used is in HDF5 format. An example of \inltt{meshHDF5.xml} can be seen below:

\begin{lstlisting}[style=XmlStyle]
    <NEKTAR>
        <GEOMETRY DIM="3" SPACE="3" HDF5FILE="meshHDF5.nekg">
            <EXPANSIONS>
            ...              
            </EXPANSIONS>
    </NEKTAR>
...
\begin{notebox}
  When running simulations or calling FieldConvert, the file specified for the mesh should be the \inltt{.xml} file. 
\end{notebox}
