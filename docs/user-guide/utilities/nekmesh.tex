...
\inltt{processall} and has the syntax
...
\inltt{151} respectively, containing the triangular and quadrilateral
...
as \inltt{stdout} no output file will be created. This option is typically
...
the original \inltt{bigMesh.xml}. These can however be recovered by an
output module option \inltt{chkbndcomp} in \nm. To do this one should
specify
\begin{lstlisting}[style=BashInputStyle]
...
composite with \inltt{id=9999}. This capability is currently only set up for 3D meshes
...
The prismatic layer for the linear mesh in Star-CCM should not be splitted e.g. there should be only one layer with the total thickness of the boundary layer (see figure \ref{fig:util:mc:Car_mesh}. Splitting will then be perfomed directly by \nm on surfaces specified by the argument \inltt{surf} and the number of layers (\inltt{layers} argument) and growth rate (\inltt{r} argument) are specified during the mesh conversion process with the \inltt{bl} option. Surfaces correspond to the regions created in Star-CCM and the reference index is displayed when running \nm with verbosity (\inltt{-v} option).
...
The command \inltt{linearise} can be added to correct bad elements.
