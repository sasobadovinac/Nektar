...
in which $A$ is the Area (related to pressure), $x$ is the axial coordinate along the vessel, $U(x,t)$ the axial velocity, $P(x,t)$ is the pressure in the tube,  $\rho$ is the density and finally $f$ the frictional force per unit length. The unknowns in equation \ref{eqn:1DContMom} are $u, A$ and $p$; hence we must provide an explicit algebraic relationship to close this system. Typically, closure is provided by an algebraic relationship between $A$ and $p$. The PulseWaveSolver currently has two tube laws implemented: a Voigt type viscoelastic law

\begin{equation}
p=p_{0}+\beta\left(\sqrt{A}-\sqrt{A_{0}}\right) + \frac{\Gamma}{\sqrt{A}}\frac{\partial A}{\partial t},
\label{eqn:PA}
\end{equation}
...
where $p_{0}$ is the reference pressure, $A_{0}$ is the reference cross-sectional area, $E$ is the Young's modulus, $h$ is the vessel wall thickness, $\nu$ is the Poisson's ratio, and $\varphi$ is the wall viscosity; and the power law

\begin{equation}
    \label{eqn:Power}
    p = p_{0} + \frac{2\rho c^{2}_{0}}{b}\left[\left(\frac{A}{A_{0}}\right)^{b/2} - 1\right] + \frac{\Gamma}{\sqrt{A}}\frac{\partial A}{\partial t},
\end{equation}

where $b$ is a paramater that varies between each artery, and $c_{0}$ is the wave speed at the reference pressure.

Application of Riemann's method of characteristics to the system of equations indicates that velocity and area are propagated through the system by forward and backward travelling waves. These waves are reflected and within the network by appropriate treatment of interfaces and boundaries. In the following, we will explain the usage of the blood flow solver on the basis of a single-artery problem and also on an arterial network consisting of $55$ arteries.
...
\item \inltt{PressureArea}:
    \begin{itemize}
    \item \inltt{Beta (default) uses the Voigt-type viscoelastic law}
    \item \inltt{Power uses the power law}
    \end{itemize}
...
of the session file:
\begin{itemize}
\item \inltt{MaterialProperties}: specifies the value of $\beta$ for each domain.
\item \inltt{A\textunderscore0}: specifies $A_{0}$ for each domain as used in the tube law.
\item \inltt{Viscoelasticity}: specifies the value of $\Gamma$ for each domain.
...
following lines show how we specify $A_0$, $\beta$ and $\Gamma$ for different arterial
segments. Note that $\Gamma$ is set to 0 for each segment by default, and so does not need to be specified for elastic simulations.
\begin{lstlisting}[style=XmlStyle]
...
<FUNCTION NAME="Viscoelasticity"> 
    <E VAR="gamma" DOMAIN="0" VALUE="1" />
      ...
    <E VAR="gamma" DOMAIN="54" VALUE="1" />
</FUNCTION>
\end{lstlisting}
...
% J. Mynard, One-dimensional haemodynamic modeling and wave dynamics in the entire adult circulation, (2015).
