...
equation \eqref{spm:eq:mask}. The output file can be changed to a \inltt{.vtu}
or \inltt{.dat} file in order to get a graphical representation of the $\Phi$
field. It is also possible to call the \texttt{phifile} module without options
to output the shape function if it has been defined in the session file as an
analytical function. In any case, it is important to remark that this
functionality is still under development and needs to be improved.
...
previously converted to FLD format.
