...
\begin{align}
  ...
  \int_{\Omega} \nabla q \cdot (\frac{\hat{u}}{\Delta t}
   - {\mathbf{N}}^{\star,n+1})  \nonumber \\
  & - \int_{\partial \Omega_d \bigcup \partial \Omega_0} q\, \nu (\nabla \times \nabla \times {\mathbf{u})}^{*,n+1} \cdot \mathbf{n} + \frac{\gamma_0}{\Delta t} \int_{\partial \Omega_d} q {\mathbf{w}}^{n+1} \cdot \mathbf{n} \label{eqn.weakpforcing}
\end{align}
...
The implicit solver is based on the work of \cite{Dong2010}. Following the 
previous subsections, the implicit solver handles the
advection terms implicitly. Effectively, it uses the same formulation
as the Semi-Implicit scheme presented above, but brings the advection
...
This means that there, still, is a time-step restriction on complex geometries
and large-scale simulations.

The advection operator is linearised
using the approximation $[\mathbf{u} \cdot \nabla \mathbf{u}]^{n+1} \approx 
[\tilde{\mathbf{u}} \cdot \nabla \mathbf{u}]^{n+1}$ where $\tilde{\mathbf{u}}$
is an approximate advection velocity defined as
\begin{equation}
    \tilde{\mathbf{u}}^{n+1} =
...
The final operator for the velocity solve in weak form and with Neumann boundary condition is
\begin{align}
	\int_\Omega 
    \frac{\gamma}{\nu \Delta t} \psi \mathbf{u}^{n+1} 
    +\int_\Omega \nabla \psi \cdot \nabla \mathbf{u}^{n+1} 
    &+\int_\Omega \frac{1}{\nu} \psi \tilde{\mathbf{u}}^{n+1} \cdot \nabla \mathbf{u}^{n+1} \nonumber\\
    &= 
    \frac{1}{\nu} \int_\Omega \psi \left( 
    - \frac{1}{\Delta t} \sum_q \alpha_q \mathbf{u}^{n-q}
    + \nabla p^{n+1} 
    - \mathbf{f}^{n+1} 
\right) \nonumber\\
    &- \int_\Gamma \mathbf{n} \cdot \psi \nabla \mathbf{u}^{n+1}
    , \\
	& \forall \psi \in H_0^1(\Omega)^d. \nonumber
\end{align}
where $\psi$ represents the test function.

The pressure solve for the implicit solver uses the weak form 
pressure equation which is defined in the previous section.
...
\item \inltt{GJPStabilisation}: activates the gradient jump penalty\cite{moura2022gradient} 
...
\subsubsection{Mask region}
If the \texttt{ModifiedArnoldi} driver is used, a subregion of the computational domain can be used to calculate the eigenvalues and eigenvectors by defining an unmask function. This can be used to improve the accuracy and to accelerate convergence if the computational domain is large and contains very coarse elements.
...
Each function \texttt{Unmask0} selects elements whose center satisfies C0>0 and C1>0 and C2>0, and so on. More than one unmasked function can be defined, and the finally selected region is the union of all unmasked regions.

If the unmask function is defined, both the unmasked eigenvector and masked eigenvector (with \_masked) will be output.
