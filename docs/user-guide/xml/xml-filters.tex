...
\begin{notebox}
  This filter is only supported for the \textbf{serial} incompressible 
  Navier-Stokes solvers in two and three dimensions.
\end{notebox}

This filter is use to solve transport of particles by an established 
velocity field. The implementation evaluate the forces (drag, gravity, 
and buoyancy.) over the particles and integrate their positions using an ODE
solver in time method. 
...
\begin{center}
  \begin{tabularx}{0.99\textwidth}{lllX}
    ...
    \midrule
%    \inltt{OutputFile}      & \xmark   & \texttt{session} &
%    Output file with the particle information. The extension \textit{.csv}
%     is used for the particles, and \textit{.cll.csv} for the collision.\\
    \inltt{DeltaT} & \cmark   &  &
    ...
    \inltt{Gravity} & \xmark   &  0 &
    Gravity magnitud applied on the "y" direction.   \\
    \inltt{WallBoundary} & \cmark   &  &
