...
\begin{notebox}
  Existing files are overwritten by default. To change this behaviour, enabling
  the CMake flag \inlsh{NEKTAR\_ENABLE\_BACKUPS} allows to save pre-existing
  data: in case the file already exists, for example the \inltt{chk}-file
  \inltt{session\_0.chk} is renamed to \inltt{session\_0.bak*.chk}.
\end{notebox}
