...
# Change rpath for Python libraries
find %{buildroot}/%{python3_sitearch}/NekPy/ -name *.so -exec chrpath -r /usr/%{_lib}/nektar++ {} \;
find %{buildroot}/%{python3_sitearch}/mpich/NekPy/ -name *.so -exec chrpath -r /usr/%{_lib}/mpich/lib/nektar++ {} \;
find %{buildroot}/%{python3_sitearch}/openmpi/NekPy/ -name *.so -exec chrpath -r /usr/%{_lib}/openmpi/lib/nektar++ {} \;
